...
const BlockedHospitals = ({ hospitals }) => {
  ...
  const unblockProvider = (providerName) => {
    axios
      .patch(
        `https://polar-dusk-61658.herokuapp.com/admin/unblock/${providerName}`,
        "",
        { headers: { Authorization: `${token}` } }
      )
      .then((res) => {
        console.log(res.data);
        setMessage("Hospital Unblocked Successfully");
        setAl(true);
        setTimeout(() => {
      ...
      .catch((error) => {
        console.log(error);
        setMessage("Hospital Unblock Failed");
        setAl(true);
        setSeverity("error");
        setTimeout(() => {
          window.location.reload(false);
                    ...
                    <TableRow key={index}>
                      ...
                      <TableCell>
                        {hospital.deactivated === true && (
                          <Chip label="Blocked" style={{ color: "#c62828" }} />
                        )}
                      </TableCell>
                      <TableCell>
                        <Button
                          ...
                          onClick={() => {
                            if (
                              window.confirm(
                                "Are you sure you want to Unblock this Hospital?"
                              )
                            )
                              unblockProvider(hospital.providerName);
                          }}
                        >
                          Unblock Account
                        </Button>
